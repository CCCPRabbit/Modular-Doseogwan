...
#define CS_LOW()		PORTD &= ~(1 << PD7)		/* Set MMC_CS = low */
#define	CS_HIGH()		PORTD |= (1 << PD7)			/* Set MMC_CS = high */
...
#define	FCLK_SLOW()		SPI_SET_BAUDRATE(400000)	/* Set SPI clock for initialization (100-400kHz) */
#define	FCLK_FAST()		SPI_SET_BAUDRATE(25000000)	/* Set SPI clock for read/write (20MHz max) */
...
{
	CS_HIGH();		/* Set CS# high */
	SPITransmitByte(0xFF);	/* Dummy clock (force DO hi-z for multiple slave SPI) */
}
...
{
	CS_LOW();		/* Set CS# low */
	SPITransmitByte(0xFF);	/* Dummy clock (force DO enabled) */
...
)
{
	...
	do {							/* Wait for data packet in timeout of 200ms */
		token = SPITransmitByte(0xFF);
	} while ((token == 0xFF) && Timer1);
	if (token != 0xFE) return 0;	/* If not valid data token, retutn with error */

	SPIReceive(buff, btr,0xff);		/* Receive the data block into buffer */
	SPITransmitByte(0xFF);					/* Discard CRC */
...
)
{
	...
	SPITransmitByte(token);					/* Xmit data token */
	if (token == 0xFD) return 1;		/* Do not send data if token is StopTran */

	SPITransmit(buff, 512);			/* Data */
	SPITransmitByte(0xFF); SPITransmitByte(0xFF);		/* Dummy CRC */

	resp = SPITransmitByte(0xFF);				/* Receive data resp */
...
)
{
	...
	/* Send command packet */
	SPITransmitByte(0x40 | cmd);				/* Start + Command index */
	SPITransmitByte((BYTE)(arg >> 24));		/* Argument[31..24] */
	SPITransmitByte((BYTE)(arg >> 16));		/* Argument[23..16] */
	SPITransmitByte((BYTE)(arg >> 8));			/* Argument[15..8] */
	SPITransmitByte((BYTE)arg);				/* Argument[7..0] */
	...
	if (cmd == CMD12) SPITransmitByte(0xFF);		/* Skip a stuff byte when stop reading */
	n = 10;								/* Wait for a valid response in timeout of 10 attempts */
	do
		res = SPITransmitByte(0xFF);
	while ((res & 0x80) && --n);
...
DSTATUS mmc_disk_initialize (void)
{
	...
	FCLK_SLOW();
	for (n = 10; n; n--) SPITransmitByte(0xFF);	/* 80 dummy clocks */

	ty = 0;
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI mode */
		Timer1 = 100;						/* Initialization timeout of 1000 msec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* Is the card SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = SPITransmitByte(0xFF);	/* Get trailing return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* The card can work at vdd range of 2.7-3.6V */
				while (Timer1 && send_cmd(ACMD41, 1UL << 30));	/* Wait for leaving idle state (ACMD41 with HCS bit) */
				if (Timer1 && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = SPITransmitByte(0xFF);
...
)
{
		...
		if (CardType & CT_SDC2) {	/* SDv2? */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				...
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					for (n = 64 - 16; n; n--) SPITransmitByte(0xFF);	/* Purge trailing data */
	...
	case MMC_GET_OCR :		/* Receive OCR as an R3 resp (4 bytes) */
		if (send_cmd(CMD58, 0) == 0) {	/* READ_OCR */
			for (n = 4; n; n--) *ptr++ = SPITransmitByte(0xFF);
			res = RES_OK;
		}
		deselect();
		break;
	...
	case ISDIO_READ:
		...
		if (send_cmd(CMD48, 0x80000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | ((sdi->ndata - 1) & 0x1FF)) == 0) {
			for (Timer1 = 100; (rc = SPITransmitByte(0xFF)) == 0xFF && Timer1; ) ;
			if (rc == 0xFE) {
				for (bp = sdi->data, dc = sdi->ndata; dc; dc--) *bp++ = SPITransmitByte(0xFF);
				for (dc = 514 - sdi->ndata; dc; dc--) SPITransmitByte(0xFF);
				res = RES_OK;
			}
		}
		deselect();
	...
	case ISDIO_WRITE:
		...
		if (send_cmd(CMD49, 0x80000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | ((sdi->ndata - 1) & 0x1FF)) == 0) {
			SPITransmitByte(0xFF); SPITransmitByte(0xFE);
			for (bp = sdi->data, dc = sdi->ndata; dc; dc--) SPITransmitByte(*bp++);
			for (dc = 514 - sdi->ndata; dc; dc--) SPITransmitByte(0xFF);
			if ((SPITransmitByte(0xFF) & 0x1F) == 0x05) res = RES_OK;
		}
		deselect();
		break;
	...
	case ISDIO_MRITE:
		...
		if (send_cmd(CMD49, 0x84000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | sdi->ndata >> 8) == 0) {
			SPITransmitByte(0xFF); SPITransmitByte(0xFE);
			SPITransmitByte(sdi->ndata);
			for (dc = 513; dc; dc--) SPITransmitByte(0xFF);
			if ((SPITransmitByte(0xFF) & 0x1F) == 0x05) res = RES_OK;
		}
		deselect();
		break;
